...
Deal Room by Aragon Holdings | Version 1.8
...
| 1.7 | Backend engineering review (Claude Code). Seven concerns resolved: (1) bulk write pattern specified — single INSERT...ON CONFLICT per table per run, not row-by-row; period-level output returned in API response, frontend holds in memory, only summaries + snapshots persisted; (2) scenario_id duality clarified — line items are templates, values are engine outputs, explicit rules documented; (3) engine restructured as modular pipeline of named pure functions; (4) XIRR upgraded to bisection-first + Newton-Raphson refinement with convergence failure handling; (5) updated_at + updated_by added to all key input tables; (6) recapitalization made strictly read-only — forks model state in memory, never writes to stored corkscrews; (7) model_access table added for multi-user RLS, mirrors existing deal_access pattern. Four open questions answered and documented. |
| 1.8 | Added versioned publish workflow. Draft mode stays non-persistent (fast iteration). Publish writes complete period output to model_published_versions (immutable, numbered, named). Investors read from published versions only, never draft state. Version comparison view shows input diffs and metric impact between any two versions. Investor dashboard with frozen-column drill-down grid. Investor invite flow. Publish notification system. Partial reversal of v1.7 table removal — draft output stays in-memory, published output persists. |
...
## 28. Backend Engineering Decisions (v1.7)

This section resolves all seven concerns raised in the Claude Code backend review. These decisions are binding — they override any conflicting language in earlier sections.

---

### 28.1 Decision: Bulk Writes, Not Row-by-Row

**Concern:** `model_values` can reach ~260K rows per calculate run (120 periods × entities × line items × scenarios × operating scenarios). Row-by-row upserts will time out.

**Decision:** The engine computes everything in memory and returns the full result object. The route handler performs a single bulk `INSERT ... ON CONFLICT DO UPDATE` per output table at the end of the run. Never loop-insert.

**Implementation pattern:**
```js
// Route handler — after engine returns
const { plRows, bsRows, cfRows, corkscrewRows, covenantRows } = engineResult

await supabase.from('model_values').upsert(plRows, {
  onConflict: 'line_item_id,scenario_id,operating_scenario_id,period_index'
})
await supabase.from('model_balance_sheet_values').upsert(bsRows, { onConflict: '...' })
// etc.
```

Batch size limit: if any table exceeds 10,000 rows in a single upsert, chunk into 5,000-row batches. Supabase has a default request body size limit.

---

### 28.2 Decision: Period Output Returned in Response, Summaries Persisted

**Concern:** Persisting all 120-period output for every scenario combination is a large write with questionable value if the frontend can hold the data in memory.

**Decision:**

| Data | Where it lives | Rationale |
|---|---|---|
| Full period-level P&L, BS, CF grid | API response only — frontend holds in memory | Fast, no storage overhead, recalculated on next Run Model |
| Debt corkscrew per instrument | API response only | Same rationale |
| Covenant results (per period) | Persisted to `model_covenant_results` | Needed for certificate generation without re-running engine |
| Snapshot JSONB | Persisted to `model_snapshots` | Frozen artifacts — must survive model changes |
| Calculate run summary | Persisted to `model_calculate_runs` (new, see 28.7) | Audit trail, staleness detection |

**Frontend implication:** The frontend must cache the full engine response in component state after each Run Model. If the user navigates away and returns, the app re-fetches from the last calculate run summary and prompts "Model results not loaded — Run Model to view." This is acceptable UX given the calculation is fast (<2 seconds for typical models).

**What this removes from the schema:** `model_values`, `model_balance_sheet_values`, and `model_cf_values` are **removed as persistent tables**. The engine returns these as structured objects in the response. Only `model_covenant_results` and `model_snapshots` persist period-level data.

This is a significant simplification. The schema is cleaner, writes are faster, and the source of truth for financial outputs is always a fresh calculate run.

---

### 28.3 Decision: scenario_id Duality Resolved

**Concern:** `model_line_items.scenario_id` and `model_values.scenario_id` serve different purposes but the rules weren't explicit.

**Decision — Two explicit rules, no exceptions:**

**Rule 1: `model_line_items.scenario_id` = line item existence scope**
- `NULL` = this line item exists in all deal structure scenarios (shared template)
- A specific `scenario_id` = this line item only exists in that scenario (e.g., a seller note interest line that only exists in the "Seller Note 30%" scenario)
- The engine uses this to determine which line items to include when building the entity P&L for a given scenario

**Rule 2: `model_values.scenario_id` = which scenario produced this value**
- Always explicit — never NULL in `model_values`
- Even if a line item has `scenario_id = NULL` (shared), its computed values in `model_values` carry the specific scenario_id of the run that produced them
- This means a shared revenue line item has N rows in `model_values` — one per scenario combination

**Conflict resolution:** If a line item has `scenario_id = X` and the engine is running scenario Y, that line item is skipped entirely for scenario Y. The engine never reads `model_line_items.scenario_id` to look up values — it uses it only to filter which line items are in scope for the current run.

**`model_values` is now removed** (per Decision 28.2), so this duality only matters for line item scoping in the engine input assembly. The route handler filters `model_line_items` by `scenario_id IS NULL OR scenario_id = :currentScenarioId` before passing to the engine.

---

### 28.4 Decision: Engine as Modular Pipeline

**Concern:** A monolithic `calculate()` function is untestable and fragile.

**Decision:** `modelEngine.js` is structured as a named pipeline of small pure functions. Each function takes a typed input and returns a typed output grid. No function has side effects.

**Canonical pipeline structure:**

```js
// Each function: (input) => grid[entityId][periodIndex] = value

generatePeriods(model)                          → periods[]

// Per entity, in close_period_index order:
projectRevenue(entity, periods, drivers)        → revenueGrid
projectCogs(entity, revenueGrid, drivers)       → cogsGrid
projectOpex(entity, periods, drivers, comp)     → opexGrid
buildPL(revenueGrid, cogsGrid, opexGrid)        → plGrid

runDebtCorkscrew(instrument, startPeriod,
  terms, periods)                               → corkscrewGrid
applyDebtToPL(plGrid, corkscrewGrids)           → plGridWithInterest

deriveWorkingCapital(plGrid, drivers, periods)  → wcGrid
deriveCapex(plGrid, drivers)                    → capexGrid

buildBalanceSheet(plGrid, wcGrid, capexGrid,
  corkscrewGrids, priorBSGrid, closeTerms)      → bsGrid

buildCashFlow(plGrid, wcGrid, capexGrid,
  corkscrewGrids, bsGrid, priorBSGrid)          → cfGrid

validateStatements(bsGrid, cfGrid)              → validationResult[]

calculateDSCR(plGrid, corkscrewGrids)           → dscrGrid
checkCovenants(dscrGrid, bsGrid, covenants)     → covenantResults[]

// Consolidation:
consolidateEntities(entityResults[])            → consolidatedGrids

// Waterfall (called separately by liquidity event route):
allocateEVByEBITDA(entityResults, exitEvent)    → spvAllocations
runSPVWaterfall(spvAllocation, equityTable)     → waterfallResult
calculateXIRR(cashFlows[])                      → irrResult
```

**Testing requirement:** Every function in this pipeline must have a corresponding unit test with known inputs and expected outputs before the calculate route is built. The engine is the most business-critical code in the application — test coverage is non-negotiable.

---

### 28.5 Decision: XIRR Implementation

**Concern:** Newton-Raphson alone has known convergence failures with multiple sign changes in cash flow series.

**Decision:** Two-phase approach:

**Phase 1 — Bisection (find bracket):**
```
Search rate range: -99% to +1000% in 0.5% steps
Find two rates r_low, r_high where NPV(r_low) and NPV(r_high) have opposite signs
If no bracket found → return { irr: null, converged: false, warning: 'no_solution' }
```

**Phase 2 — Newton-Raphson refinement:**
```
Starting from midpoint of bracket
Iterate up to 100 times
Convergence threshold: |NPV| < 0.01 (one cent)
If no convergence after 100 iterations → return bisection midpoint with warning: 'approximate'
```

**NPV formula (XIRR):**
```
NPV = Σ cashFlow[i] / (1 + rate)^((date[i] - date[0]) / 365)
```

**Edge cases:**
- All cash flows same sign → return `{ irr: null, warning: 'no_sign_change' }`
- Single cash flow → return `{ irr: null, warning: 'insufficient_flows' }`
- IRR > 1000% → return `{ irr: null, warning: 'exceeds_range' }` — these are typically data errors

**MOIC is always returned regardless of IRR convergence:**
```
MOIC = sum(positive cash flows) / sum(abs(negative cash flows))
```

---

### 28.6 Decision: Audit Fields on Key Input Tables

**Concern:** No way to see what changed if model inputs are updated between runs.

**Decision:** Add `updated_at` and `updated_by` (FK → profiles.id) to these tables:
...
All already have `created_at`. Add `updated_at` with `DEFAULT now()` and a Postgres trigger to auto-update on row change. Add `updated_by` as nullable FK.

**`model_calculate_runs` — new table (minimum audit trail):**
...
| `scenario_ids` | uuid[] | Which scenarios were run |
| `operating_scenario_ids` | uuid[] | Which operating scenarios were run |
| `run_at` | timestamptz | When the run completed |
| `duration_ms` | integer | Engine execution time |
| `passed` | boolean | True if all validation checks passed |
| `warning_count` | integer | |
| `error_count` | integer | |
| `model_version_hash` | text | Hash of all input table checksums at run time |
| `summary` | jsonb | Full per-run summary object from engine response |

The `model_version_hash` on `model_calculate_runs` is what `model_snapshots.model_version_hash` compares against for staleness detection. If the most recent calculate run's hash differs from a snapshot's hash, the snapshot is stale.

**Full audit log** (every field change tracked) is a v2 feature. The `updated_at` + `updated_by` fields plus `model_calculate_runs` are sufficient for v1.

---

### 28.7 Decision: Recapitalization is Read-Only

**Concern:** Running a recap exit event as currently specced would mutate stored corkscrews, corrupting subsequent full exit calculations.

**Decision:** Recapitalization calculations **never write to any database table**. They are computed entirely in memory and returned in the API response.

**How recap works post-decision:**

1. Route handler loads the current model state (entity financials, debt corkscrews, BS values at `recap_period_index`) from the last calculate run summary in `model_calculate_runs.summary`
2. Engine forks the debt state in memory — creates a copy of all corkscrew balances at `recap_period_index`
3. Applies new facility terms to the forked state for periods `recap_period_index → 119`
4. Computes dividend distribution
5. Returns the full forked projection and waterfall in the response
6. **Writes nothing to the database**

The stored `model_debt_corkscrews` (now removed per Decision 28.2) and the base calculate run are completely untouched.

**Implication for combined IRR (recap + eventual exit):**

The frontend holds both the base projection (from the last calculate run) and the recap projection (from the recap response). The combined IRR is computed client-side:

```
Cash flows for combined IRR:
  − contributed_capital         at contribution date
  + recap_dividend              at recap_period date
  + full_exit_proceeds          at exit_period date (from base projection)
```

XIRR on these three cash flows gives the true combined return. The frontend passes these to a lightweight XIRR utility function (can be the same implementation as the engine's, extracted to a shared util).

**"What if I want to model post-recap operations?"** — Create a new model. The recap scenario is a decision point, not a continuation of the existing model. The new model starts at the recap period with the new debt terms. This is the correct mental model: a recap is an exit from the current capital structure and entry into a new one.

---

### 28.8 Decision: Multi-User Access Control (RLS)

**Concern:** New tables have no RLS policies. The existing app uses `deal_access` for deal-level permissions — the models module needs the same pattern.

**Decision:** Add `model_access` table mirroring `deal_access`:
...
**RLS policies on `financial_models`:**
```sql
-- Select: user must have a model_access row OR be the creator
CREATE POLICY "model_select" ON financial_models
  FOR SELECT USING (
    created_by = auth.uid()
    OR EXISTS (
      SELECT 1 FROM model_access
      WHERE model_id = id AND profile_id = auth.uid()
    )
  );

-- Insert: authenticated users can create models
CREATE POLICY "model_insert" ON financial_models
  FOR INSERT WITH CHECK (created_by = auth.uid());

-- Update/Delete: owner or editor only
CREATE POLICY "model_update" ON financial_models
  FOR UPDATE USING (
    created_by = auth.uid()
    OR EXISTS (
      SELECT 1 FROM model_access
      WHERE model_id = id AND profile_id = auth.uid() AND role IN ('owner', 'editor')
    )
  );
```

**All child tables** (`model_entities`, `model_scenarios`, `model_deal_references`, etc.) inherit access control via their FK to `financial_models.id`. Their RLS policies follow the same pattern — check model_access via a join.

**Auto-grant on create:** When a model is created, the route handler immediately inserts a `model_access` row with `role = 'owner'` for the creating user.

**Render backend:** The calculate route and snapshot route authenticate via the existing Supabase service role key. They bypass RLS intentionally — the route handler validates that the requesting user has at minimum `'editor'` access before calling the engine.

---

### 28.9 Open Questions — Answered

**Q1: How to handle recap mutability?**
Answered in 28.7. Recaps are read-only projections, never mutations. Post-recap operations require a new model.

**Q2: Persist all period output or return in response?**
Answered in 28.2. Period output returned in response, frontend holds in memory. Only covenant results, calculate run summaries, and snapshots persisted.

**Q3: Can the consolidated entity have its own line items?**
No. The consolidated entity is always auto-computed — sum of all active entity results at each period, filtered to `revenue_type = 'client'` for the top-line revenue figure. It never has rows in `model_line_items`. Its presence in `model_entities` is for UI tab rendering only. The engine never processes it as an input entity — only as an output aggregation.

**Q4: Backend build order?**
Six-phase tightened sequence below (Section 29).

---

## 29. Backend Build Sequence (Tightened)

Six phases. Each phase has a clear done condition before the next starts.

### Phase 1 — Schema and Access Control
All migrations. `model_access` table. RLS policies on all new tables. `updated_at` triggers on all input tables. `model_calculate_runs` table.

**Done when:** All tables exist, constraints enforced, RLS policies tested with two test users (owner can read/write, non-member gets 0 rows).

---

### Phase 2 — CRUD Routes
Direct Supabase CRUD for all input tables. No engine involvement yet. Routes:
- Models: create, read, update, delete
- Entities: auto-create on model create, update name
- Scenarios (deal + operating): CRUD
- Deal references: CRUD, `close_period_index` calculated on insert
- Deal scenario terms: CRUD
- Line items: CRUD with `scenario_id` scoping rules enforced
- Drivers: CRUD
- SPV equity: CRUD, 100% validation on base_equity_pct
- Holdco compensation: CRUD
- Covenants: CRUD, auto-generate standard set on model create
- COA imports: CRUD

**Done when:** Full model can be configured via API calls with no engine involvement. Postman collection covers all routes.

---

### Phase 3 — Engine Core (P&L only)
Build the modular pipeline through `buildPL()` and `applyDebtToPL()`. No balance sheet or cash flow yet.

Route: `POST /api/models/:modelId/calculate` — returns P&L grids per entity per scenario combination. No DB writes yet except `model_calculate_runs`.

Unit tests for every pipeline function before wiring to the route.

**Done when:** Known inputs (hardcoded test model) produce correct P&L values for all periods. DSCR calculated correctly. Covenant P&L-based checks pass.

---

### Phase 4 — Engine: Balance Sheet and Cash Flow
Add `buildBalanceSheet()`, `buildCashFlow()`, `validateStatements()`. Add `deriveWorkingCapital()` and `deriveCapex()`.

Route returns full three-statement output. Bulk write to `model_covenant_results`. Write to `model_calculate_runs`.

**Done when:** Balance sheet balances every period. CF ending cash ties to BS cash every period. Any period where statements don't tie returns an error (not a warning).

---

### Phase 5 — Liquidity Event Route
`GET /api/models/:modelId/liquidity-event/:scenarioId/:operatingScenarioId/:exitEventId`

Loads last calculate run summary from `model_calculate_runs`. Runs `allocateEVByEBITDA()`, `runSPVWaterfall()`, `calculateXIRR()` per party per SPV. Returns full waterfall and return metrics. No DB writes.

Recap variant: forks debt state in memory, returns projected dividend and post-recap trajectory. No DB writes.

**Done when:** Known model (manual calculation verified) produces correct MOIC and IRR for each party in each SPV. Recap dividend calculates correctly. Bisection + Newton-Raphson converges on correct IRR.

---

### Phase 6 — Snapshot Route
`POST /api/models/:modelId/snapshot`

Loads period data from last calculate run summary. Assembles S&U, DSCR, debt state, BS exhibits. Writes to `model_snapshots` on save. Computes `model_version_hash` and compares to stored hash for staleness.

**Done when:** Snapshot saved at period 0 and period 12. Reopened snapshot shows frozen data. Model input change → staleness banner fires correctly.

---

## 30. Schema Summary — What Persists vs. What Doesn't

Given Decision 28.2, here is the definitive list of what lives in the database vs. what lives in API responses:

**Persisted to database (source of truth):**
- All input tables: `financial_models`, `model_scenarios`, `model_operating_scenarios`, `model_entities`, `model_segments`, `model_deal_references`, `model_deal_scenario_terms`, `model_line_items`, `model_drivers`, `model_spv_equity`, `model_holdco_compensation`, `model_covenants`, `coa_imports`, `mid_baseline`, `mid_baseline_entities`, `model_access`
- Run history: `model_calculate_runs` (summary + hash only, not full grids)
- Covenant compliance: `model_covenant_results` (needed for certificate generation)
- Snapshots: `model_snapshots` (frozen JSONB artifacts)
- Deal references: `model_deal_references`, `model_liquidity_events`

**Not persisted — returned in API response, held in frontend memory:**
- P&L grids (120 periods × all entities × all scenario combinations)
- Balance sheet values (120 periods × all entities)
- Cash flow statement values (120 periods × all entities)
- Debt corkscrew period detail (120 periods × all instruments)
- Liquidity event waterfall results
- Recap projections

**Removed from schema (previously specified, now removed):**
- `model_values` — replaced by API response pattern
- `model_balance_sheet_values` — replaced by API response pattern
- `model_cf_values` — replaced by API response pattern
- `model_debt_corkscrews` — replaced by API response pattern (corkscrew summaries included in calculate run summary JSONB)

---

---

## 31. Versioned Publish Workflow

### 31.1 The Core Mental Model

The application has two completely separate states:

**Draft state** — where you build and iterate. Fast. Non-persistent period output. Only you (and editors) can see this. Recalculates in memory on demand. This is your working spreadsheet.

**Published state** — what investors see. Immutable. Full period output written to the database. Versioned and numbered. Investors can only ever see published versions — never your draft work-in-progress.

```
YOUR WORKFLOW:
  Draft → iterate → draft → iterate → draft → iterate
                                              ↓
                                           PUBLISH
                                              ↓
                                       Version 3 created
                                       Investors notified
                                       Full output frozen in DB
```

This replicates exactly how you'd manage this today with spreadsheets — you have your working file, and when you're ready you send a specific version. The difference is investors get a link to a live portal showing that version, not an attachment.

### 31.2 Schema: model_published_versions

One row per publish action. Stores complete frozen output as JSONB.

| Column | Type | Nullable | Notes |
|---|---|---|---|
| `id` | uuid | No | PK |
| `model_id` | uuid | No | FK → financial_models.id, ON DELETE CASCADE |
| `version_number` | integer | No | Auto-incrementing per model. 1, 2, 3... |
| `version_name` | text | No | User-provided. e.g. 'Initial Model', 'Revised — Lower Price', 'Post-QoE' |
| `version_notes` | text | Yes | What changed in this version. Shown to investors. |
| `published_by` | uuid | No | FK → profiles.id |
| `published_at` | timestamptz | No | now() |
| `scenario_id` | uuid | No | Which deal structure scenario is the featured/default view |
| `operating_scenario_id` | uuid | No | Which operating scenario is the featured/default view |
| `model_inputs_hash` | text | No | Hash of all input tables at publish time — used for version diff |
| `pl_output` | jsonb | No | Full P&L grid: all entities × all scenarios × 120 periods |
| `bs_output` | jsonb | No | Full balance sheet grid |
| `cf_output` | jsonb | No | Full cash flow grid |
| `corkscrew_output` | jsonb | No | Full debt corkscrew grid |
| `covenant_output` | jsonb | No | Covenant compliance per period |
| `waterfall_output` | jsonb | No | Exit event waterfalls per scenario combination |
| `key_metrics` | jsonb | No | Summary metrics — DSCR, MOIC, IRR per party, equity pool. Used for version comparison and dashboard. |
| `input_snapshot` | jsonb | No | Frozen copy of all input table values at publish time. Used for version diff display. |
| `is_current` | boolean | No | True for the most recently published version. Only one true per model. |
| `investor_visible` | boolean | No | Default true. Set false to hide a version from investors without deleting it. |

**Unique constraint:** `UNIQUE(model_id, version_number)`

**Why JSONB for output:** Each published version is a self-contained artifact. JSONB means it's readable years later regardless of schema changes. The version is immutable — it never needs to be queried relationally, only retrieved and rendered.

**Size estimate:** A typical model with 6 entities, 3 deal scenarios, 3 operating scenarios, 120 periods, 25 line items per entity produces roughly 2-4MB of JSONB per publish. Very manageable.

### 31.3 Schema: model_version_access

Controls which investors can see which versions. By default, all viewers with `model_access` can see all published versions. But you may want to show Version 2 to Investor A before you're ready to show it to Investor B.

| Column | Type | Notes |
|---|---|---|
| `id` | uuid | PK |
| `version_id` | uuid | FK → model_published_versions.id, ON DELETE CASCADE |
| `profile_id` | uuid | FK → profiles.id |
| `granted_at` | timestamptz | |
| `granted_by` | uuid | FK → profiles.id |

If this table has no rows for a version, the default behavior applies (all model viewers can see it). If any rows exist, only those specific profiles can see it — override mode.
...
```json
{
  "versionName": "Post-QoE Updated",
  "versionNotes": "Revised Agency A purchase price to $4.8M based on QoE findings. Updated facility rate to 8.75%. Agency B close moved to month 9.",
  "featuredScenarioId": "uuid",
  "featuredOperatingScenarioId": "uuid",
  "notifyInvestors": true
}
```

**Route handler steps:**
1. Verify requesting user has `owner` or `editor` role on the model
2. Run full engine calculate (same as draft calculate but writes to DB)
3. Compute `model_inputs_hash` from all input tables
4. Assemble `input_snapshot` JSONB from all current input table values
5. Assemble `key_metrics` summary object
6. Set `is_current = false` on all prior versions for this model
7. Insert new `model_published_versions` row with all output
8. Insert `model_calculate_runs` row for audit trail
9. If `notifyInvestors = true`, trigger notification to all viewers (email or in-app)
10. Return `{ success: true, versionNumber: N, versionId: uuid }`

**Response time expectation:** 3-8 seconds including engine run + DB write. Acceptable for a deliberate publish action — show a loading state with "Publishing version N..." messaging.

### 31.5 Draft vs. Publish Routes Summary

```
POST /api/models/:modelId/calculate
  → Engine runs in memory
  → Returns full output in response
  → Writes only: model_calculate_runs, model_covenant_results
  → Who can trigger: owner, editor
  → Investors see: nothing (draft only)

POST /api/models/:modelId/publish
  → Engine runs in memory
  → Returns full output in response (same as calculate)
  → Writes: model_published_versions (full JSONB output)
             model_calculate_runs
             model_covenant_results
  → Who can trigger: owner, editor
  → Investors see: this version immediately after publish

GET /api/models/:modelId/versions
  → Returns list of all published versions (metadata only, no JSONB)
  → Filtered by model_version_access if override rows exist
  → Who can access: owner, editor, viewer

GET /api/models/:modelId/versions/:versionId
  → Returns full published version JSONB
  → Who can access: owner, editor, viewer (with access)
```
...
### 32.1 What Investors Actually Need to See

When an investor asks "what changed between version 2 and version 3?", they need two things:

1. **Input changes** — what assumptions did you change (the why)
2. **Metric impact** — how did those changes affect their return (the so what)

### 32.2 Version Diff Computation

The diff is computed by the frontend by comparing `input_snapshot` JSONB between two versions. No additional route needed — the snapshots already contain everything.

**Input diff algorithm:**
```js
function diffVersionInputs(versionA, versionB) {
  const inputsA = versionA.input_snapshot
  const inputsB = versionB.input_snapshot
  
  return {
    changed: findChangedFields(inputsA, inputsB),   // fields with different values
    added: findAddedFields(inputsA, inputsB),        // fields in B not in A
    removed: findRemovedFields(inputsA, inputsB)     // fields in A not in B
  }
}
```

**Key metrics diff** — straightforward comparison of `key_metrics` JSONB between versions:
```js
{
  dscr_min:         { v2: 1.56, v3: 1.71, delta: +0.15, direction: 'better' },
  tristan_moic:     { v2: 2.9,  v3: 3.4,  delta: +0.5,  direction: 'better' },
  investor_a_moic:  { v2: 4.5,  v3: 5.1,  delta: +0.6,  direction: 'better' },
  equity_pool_m36:  { v2: 13.4, v3: 15.2, delta: +1.8,  direction: 'better' }
}
```

### 32.3 Version Comparison UI

Accessible from both builder mode and investor view. A dedicated comparison panel:
...
Version Notes: "Revised Agency A purchase price based on QoE findings..."
```
...
Min DSCR                  1.56×        1.71×      ↑ better
Tristan — MOIC            2.9×         3.4×       ↑ better
Tristan — IRR             52%          58%        ↑ better
Investor A — MOIC         4.5×         5.1×       ↑ better
Investor A — IRR          38%          43%        ↑ better
Month 36 Equity Pool      $13.4M       $15.2M     ↑ +$1.8M
```

**Section 3 — Side-by-side period view (optional drill-down):**
Two frozen-column grids side by side, same period visible, showing divergence in key line items period by period. For power users only — most investors just need sections 1 and 2.

---

## 33. Investor Dashboard and Drill-Down View

### 33.1 Access Flow

When a viewer logs in and navigates to the Models section, they see only the models they have access to. Clicking a model takes them to the Investor View — a completely different layout from the builder's ModelBuilder.

```
URL structure:
  Builder:   /models/:modelId                    → ModelBuilder
  Investor:  /models/:modelId/view               → InvestorView
  Version:   /models/:modelId/view?v=3            → InvestorView at version 3
  Compare:   /models/:modelId/compare?a=2&b=3    → CompareView
```

The app detects the user's role from `model_access` and routes accordingly. An owner/editor visiting `/models/:modelId/view` sees the investor view as a preview mode.

### 33.2 Investor View Layout

```
┌─────────────────────────────────────────────────────────────┐
│  [Model Name]          Version 3 — Post-QoE    [History ▾]  │
│  Published Feb 10, 2026 by Tristan Pelligrino               │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  PLATFORM SUMMARY          YOUR POSITION                    │
│  ─────────────────         ──────────────────               │
│  Portfolio EBITDA  $5.5M   Entity:     SPV-B               │
│  Companies:        4       Stake:      20% base            │
│  Hold period:      36mo    Committed:  $750,000            │
│  Min DSCR:         1.71×   MOIC:       5.1× (base)         │
│                            IRR:        43% (base)          │
│                                                             │
├─────────────────────────────────────────────────────────────┤
│  SCENARIO  [Base (Management) ▾]  [Seller Note Structure ▾] │
├─────────────────────────────────────────────────────────────┤
│  [Summary] [P&L Drill-Down] [Exit Scenarios] [Documents]    │
└─────────────────────────────────────────────────────────────┘
```

### 33.3 P&L Drill-Down Tab — The Frozen Grid

This is the full 120-column grid that answers your original question. Built to feel like a spreadsheet — frozen left column, horizontal scroll.

**Grid structure:**
```
┌──────────────────────────┬──────┬──────┬──────┬──────┬─────
│ LINE ITEM                │ M 1  │ M 2  │ M 3  │ M 4  │ ...
├──────────────────────────┼──────┼──────┼──────┼──────┼─────
│ ▼ REVENUE                │      │      │      │      │
│   AEO/SEO Retainers      │ 125K │ 128K │ 131K │ 134K │ ...
│   Paid Media Mgmt Fees   │  42K │  43K │  44K │  45K │ ...
│   Referral Income        │  16K │  16K │  17K │  17K │ ...
│   Total Revenue          │ 183K │ 187K │ 192K │ 196K │ ...
├──────────────────────────┼──────┼──────┼──────┼──────┼─────
│ ▼ COGS                   │      │      │      │      │
│   ...                    │      │      │      │      │
├──────────────────────────┼──────┼──────┼──────┼──────┼─────
│   GROSS PROFIT           │ 134K │ 137K │ 140K │ 143K │ ...
│   Gross Margin %         │ 73%  │ 73%  │ 73%  │ 73%  │ ...
├──────────────────────────┼──────┼──────┼──────┼──────┼─────
│   ...                    │      │      │      │      │
├──────────────────────────┼──────┼──────┼──────┼──────┼─────
│   EBITDA                 │  62K │  64K │  66K │  68K │ ...
│   DSCR                   │ 1.82 │ 1.79 │ 1.76 │ 1.73 │ ...
└──────────────────────────┴──────┴──────┴──────┴──────┴─────
```

**Grid behaviors:**
- Left column (line item labels + entity names) is CSS `position: sticky, left: 0` — never scrolls
- Column headers (month labels) are `position: sticky, top: 0` — stay visible on vertical scroll
- Presentation granularity toggle: Monthly / Quarterly / Annual collapses columns client-side
- Entity selector: tabs above the grid to switch between entities or view consolidated
- DSCR row: amber background below 1.25×, red below 1.00×
- Pre-close periods: grayed cells with no values
- Acquisition close markers: small flag icon on the column header at each close period
- Read-only: no cells are editable in investor view

**Performance note:** 120 columns × ~50 rows = 6,000 cells per entity view. Render all cells — no virtualization needed at this size. Use CSS grid or a simple HTML table. Do not use a heavy grid library (AG Grid, Handsontable) — they're overkill and add bundle weight for a mostly-read use case.

### 33.4 Exit Scenarios Tab

```
EXIT SCENARIOS — Your Position (Investor A, SPV-B, 20% base)
──────────────────────────────────────────────────────────────
...
Platform EBITDA:    $5.5M           $7.2M           $5.5M
Exit Multiple:      7.0×            7.0×            5.0× refi
Gross EV:           $38.5M          $50.4M          $27.5M new debt

Your gross return:  $3.85M          $6.65M          $2.05M dividend
Your MOIC:          5.1×            8.9×            2.7× partial
Your IRR:           43%             38%             — (partial exit)

Tristan's return:   $9.8M           $17.2M          $5.3M dividend
Tristan's MOIC:     3.4×            5.9×            —

──────────────────────────────────────────────────────────────
[Switch scenario ▾]  [Download as PDF]
```

### 33.5 Documents Tab (Snapshot Library)

```
PUBLISHED DOCUMENTS
──────────────────────────────────────────────────────────────
📄 Agency A Close — Sources & Uses        Jan 15, 2026  [View] [PDF]
📄 Agency B Close — Sources & Uses        Mar 02, 2026  [View] [PDF]
📄 Q1 2026 Covenant Certificate           Apr 01, 2026  [View] [PDF]
📄 Platform Summary — Month 18            Jul 15, 2026  [View] [PDF]

VERSION HISTORY
──────────────────────────────────────────────────────────────
v3  Post-QoE Updated           Feb 10, 2026  [View] [Compare to v2]
v2  Revised — Lower Price       Jan 28, 2026  [View] [Compare to v1]
v1  Initial Model               Jan 15, 2026  [View]
```

---

## 34. Investor Invite Flow

### 34.1 Invite from Models Module

From the ModelBuilder top bar: **Share** button → opens Share Panel.

```
SHARE MODEL
──────────────────────────────────────────────────────────────
Invite by email:  [investor@familyoffice.com        ] [Invite]

Role:  ○ Editor (can edit and publish)
       ● Viewer (read-only investor access)

Access to versions:
  ● All published versions (default)
  ○ Specific versions only → [select versions]

CURRENT ACCESS
──────────────────────────────────────────────────────────────
Tristan Pelligrino     Owner
Business Partner       Editor
Investor A             Viewer    Invited Jan 15 · Last viewed Feb 12
Investor B             Viewer    Invited Jan 28 · Last viewed Feb 10
                                 [Revoke]
```

### 34.2 Invite Mechanics

Mirrors the existing Invite Seller flow in the Deals module:

1. Enter email → system checks if profile exists in `authorized_emails` or `profiles`
2. If exists → insert `model_access` row → send notification email
3. If not exists → insert `authorized_emails` row → send invite email with signup link
4. On signup, profile is created and `model_access` row is inserted automatically

**Email content:**
```
Subject: Tristan Pelligrino shared a financial model with you

[Name] has invited you to view "AEO Roll-Up 2025" in Deal Room
by Aragon Holdings.

This model shows the acquisition strategy, financial projections,
and your projected returns as an investor.

[View Model →]
```

### 34.3 Publish Notifications

When `notifyInvestors: true` on a publish action, all viewers with `model_access` receive:

```
Subject: Model Updated — AEO Roll-Up 2025 (Version 3)

Tristan Pelligrino has published an updated version of
"AEO Roll-Up 2025".

What changed: "Revised Agency A purchase price based on QoE
findings. Updated facility rate to 8.75%."

Your projected return (Base case, Month 36 exit):
  Previous version:  4.5× MOIC, 38% IRR
  This version:      5.1× MOIC, 43% IRR

[View Updated Model →]  [Compare Versions →]
```

The notification email includes the metric delta pulled from `key_metrics` comparison — investors see immediately whether the update is better or worse for them without opening the app. That's a sophisticated touch that builds confidence.

### 34.4 Last Viewed Tracking

Add `model_access_log` table for basic engagement tracking:
...
| `profile_id` | uuid | FK → profiles.id |
| `version_id` | uuid | FK → model_published_versions.id |
| `viewed_at` | timestamptz | |
| `time_spent_seconds` | integer | Optional — set on page unload |

This powers the "Last viewed Feb 12" display in the Share Panel, giving you signal on which investors are actively reviewing the model and which haven't opened it yet. Useful context before a call.

---

## 35. Revised Schema Summary (v1.8)

### What Persists vs. What Doesn't — Final Answer

| Data | Persists? | Where | Notes |
|---|---|---|---|
| All model inputs | Yes | Input tables | Source of truth |
| Draft P&L / BS / CF output | No | API response + browser memory | Recalculate on demand |
| Published P&L / BS / CF output | Yes | `model_published_versions.pl_output` etc. (JSONB) | Immutable per version |
| Debt corkscrew (draft) | No | API response | |
| Debt corkscrew (published) | Yes | `model_published_versions.corkscrew_output` | |
| Covenant results (draft) | Yes | `model_covenant_results` | Certificate generation |
| Covenant results (published) | Yes | `model_published_versions.covenant_output` | |
| Calculate run summaries | Yes | `model_calculate_runs` | Audit + staleness |
| Snapshots | Yes | `model_snapshots` | Point-in-time exhibits |
| Published versions | Yes | `model_published_versions` | Investor-facing, immutable |
| Waterfall / exit results | Yes (published) | `model_published_versions.waterfall_output` | |
| Recap projections | No | API response only | Read-only, never persisted |
| Version access log | Yes | `model_access_log` | Engagement tracking |

### New Tables Added in v1.8

- `model_published_versions` — core of the publish system
- `model_version_access` — per-version access overrides
- `model_access_log` — investor engagement tracking

### Tables Removed (confirmed from v1.7)

- `model_values` — replaced by published JSONB + API response pattern
- `model_balance_sheet_values` — same
- `model_cf_values` — same
- `model_debt_corkscrews` — same (corkscrew detail lives in published JSONB or API response)

---

## 36. Updated Backend Build Sequence (v1.8)

The six-phase sequence from v1.7 holds. Add Phase 7:

### Phase 7 — Publish and Investor Access

**7a — Publish route:**
Wire `POST /api/models/:modelId/publish`. Full engine run + bulk JSONB write to `model_published_versions`. Increment version number. Set `is_current`. Write `model_calculate_runs`.

**Done when:** Publish creates a new version row with correct JSONB. Re-opening that version returns identical numbers to the calculate run that produced it.

**7b — Version list and retrieval routes:**
`GET /api/models/:modelId/versions` — metadata only.
`GET /api/models/:modelId/versions/:versionId` — full JSONB. RLS enforced via `model_access` + `model_version_access`.

**Done when:** Owner sees all versions. Viewer sees only versions they have access to. Non-member gets 0 rows.

**7c — Invite flow:**
Reuse existing authorized_emails pattern. Insert `model_access` row. Trigger invite email.

**Done when:** Invited email receives invite, signs up or logs in, sees investor view with published versions.

**7d — Publish notification:**
On publish with `notifyInvestors: true`, send email to all viewers with metric delta. Use existing email infrastructure.

**Done when:** Investor receives email with correct metric comparison after publish.

---

## 37. Investor View — Lovable Component Notes

The investor view (`/models/:modelId/view`) is a separate page layout from ModelBuilder. Key differences:

- No editing controls anywhere — no input fields, no Run Model button, no scenario configuration
- Scenario selector is a read-only dropdown — investor can switch between scenarios to see different projections but cannot modify scenario terms
- Version selector in the top bar — investor can switch between published versions
- Compare button — opens the version comparison view
- All data reads from `model_published_versions` JSONB, never from input tables directly
- The frozen-column grid reads the `pl_output` JSONB and renders it — no engine call needed
- "Your Position" card is personalized: reads `model_spv_equity` to find the viewing investor's equity stake and renders their specific return metrics from `waterfall_output`

**The frozen column implementation:**
```css
.grid-container {
  display: grid;
  grid-template-columns: 240px repeat(120, 80px);
  overflow-x: auto;
}
...
.line-item-label.header {
  position: sticky;
  left: 0;
  top: 0;
  z-index: 3;  /* corner cell — sticky in both directions */
}
```

Build this as a plain CSS grid or HTML table with sticky positioning. Do not use a third-party grid library.

---

*Deal Room by Aragon Holdings | M&A Financial Modeling Module | v1.8 — Complete*
